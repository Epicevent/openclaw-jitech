...
export function resolveGoogleGeminiForwardCompatModel(params: {
  ...
  if (lower.startsWith(GEMINI_2_5_PRO_PREFIX)) {
    ...
    !lower.startsWith(GEMINI_3_5_FLASH_LITE_PREFIX)
  ) {
    // gemini-3.5-flash is newer than the catalog's dedicated flash templates; resolve it via the
    // 3.x Flash lineage (falling back to 2.5-flash) so the request is forwarded to the provider
    // API under its own id. A future gemini-3.5-flash-lite is excluded here (returns undefined
