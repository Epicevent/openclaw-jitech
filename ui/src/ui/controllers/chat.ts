    ...
    for (;;) {
      try {
        res = await state.client.request<{
          ...
          // No maxChars: the per-block cap belongs to the gateway, which resolves it from
          // gateway.webchat.chatHistoryMaxChars (default 100k). Sending one here silently
          // overrode that knob and cut every block at 4k in the dashboard.
        });
        break;
      } catch (err) {
